...
class Menu {
    ...
    ManagePreparedSpellList() {
        ...
        ////this chunk is what is used to retieve the top Item "Spell list" then write out all the spells that are contained in that list.
        if (index > -1 && index < this.spellLists.length) {
            let info = this.refreshList(index); // calls refresher for updating list with real time list updating.
            console.log(info);
            let selection = this.showSpellListMenuOptions(info);
            while (selection != 0 && selection != null) {
                    switch (selection) {
                        case '1':
                            this.createSpell();
                            selection = this.showSpellListMenuOptions(this.refreshList(index));
